...
async function buildQrTickets(booking, ticketMeta) {
    ...
    const pricePerSeat = seats.length
        ? Number(booking.total_price || 0) / seats.length
...
async function sendTicketEmail(booking, user) {
    ...
    const movieTitle =
        booking.movie_title ||
        booking.show_id?.movie_id?.title ||
        'Unknown Movie';

    const showTime =
        booking.show_time ||
        booking.show_id?.show_time ||
        null;

    const hall =
        booking.theater_name ||
        booking.show_id?.theater_id?.name ||
        'Main Hall';

    const experience = screenTypeToExperience(
        booking.show_id?.theater_id?.screen_type,
        hall
    );
    ...
    try {
        ...
        const ticketHtml = tickets.map(ticket => `
            <div style="display:inline-block;width:220px;margin:10px;padding:16px;border:1px solid #ddd;border-radius:8px;text-align:center;vertical-align:top;background-color:#ffffff;">
                <div style="font-size:18px;font-weight:bold;color:#b71c1c;margin-bottom:8px;">
                    Seat ${escapeHtml(ticket.seatLabel)}
                </div>
        ...
        const htmlContent = `
            <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
                ...
                <p style="color:#666;font-size:13px;">
                    Your seat and ticket details are below. You can view or download the QR ticket from the booking confirmation page.
                </p>

                <div style="text-align:center;">
                    ${ticketHtml || '<p>No tickets were generated.</p>'}
                </div>

                <p>
                    Please present your booking reference at the counter. You can also show the QR ticket from the website confirmation page.
                </p>
