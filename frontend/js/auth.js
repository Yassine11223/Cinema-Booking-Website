 ...
 * Connects to local backend API at http://localhost:5000
 */
    ...
    if (params.get('google') === 'success') {
        ...
        try {
            ...
            if (user?.role && user.role !== 'customer') {
                showGlobalError('Please use the admin login portal to sign in.');
                return;
            }
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                window.location.href = urlRedirect || 'index.html';
            } else {
                showGlobalError(data.message || 'Invalid email or password.');
                setFieldStatus('email', false, '');
            }
        } catch (err) {
            showGlobalError('Customer login requires the backend and MongoDB to be running.');
            setFieldStatus('email', false, '');
        } finally {
...
function initRegisterForm(form) {
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            ...
            showGlobalError('Registration requires the backend and MongoDB to be running.');
        } finally {
...
function initOtpForm(otpForm, email) {
    ...
    otpForm.onsubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                if (data.user?.role && data.user.role !== 'customer') {
                    setFieldStatus('otpCode', false, 'Please use the admin login portal to sign in.');
                    return;
                }
                localStorage.setItem('userToken', data.token);
                localStorage.setItem('userData', JSON.stringify(data.user));
